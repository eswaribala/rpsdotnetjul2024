...
namespace OrderConsumerAPI.Services
{
    ...
    {
        private readonly IMongoRepository _orderRepository;
        private readonly IConfiguration _configuration;

        public ConsumerService(IMongoRepository orderRepository, IConfiguration configuration)
        {
            _orderRepository = orderRepository;
            _configuration = configuration;

        }
        protected override async Task<Task<string>> ExecuteAsync(CancellationToken stoppingToken)
        {
            await Task.Yield();
                ...
                try
                {
                    while (true)
                    {
                        try
                        {
                            ...
                            var result = JsonConvert.DeserializeObject<Order>(cr.Value);
